Instance: Bundle-IPS-FR
InstanceOf: Bundle
Usage: #example
* meta.lastUpdated = "2025-10-09T13:28:17.000+00:00"
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-bundle-document"
* identifier.system = "1.2.250.1.213.1.1.1.51.2024.1.1"
* identifier.value = "2024.01"
* type = #document
* timestamp = "2025-10-09T13:28:17.000+00:00"
* entry[0].fullUrl = "urn:uuid:09275181-4d85-43b8-89b0-6dd68182bc52"
* entry[=].resource = Composition-IPS-FR
* entry[+].fullUrl = "urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac"
* entry[=].resource = patientExemple
* entry[+].fullUrl = "urn:uuid:a11d31c5-77ff-4642-91f7-66c4d10d18c9"
* entry[=].resource = practitioner-role-exemple-1
* entry[+].fullUrl = "urn:uuid:b5941194-08be-4893-a629-652f97587b39"
* entry[=].resource = practitioner-exemple-1
* entry[+].fullUrl = "urn:uuid:579f1274-8265-4bb1-91ba-d093a11be4f5"
* entry[=].resource = organizationExemple
* entry[+].fullUrl = "urn:uuid:51807e91-cb17-4ca1-bc58-1efa85cf9d72"
* entry[=].resource = encounterExemple
* entry[+].fullUrl = "urn:uuid:648170b7-1538-44a7-8820-df04dcc32fad"
* entry[=].resource = locationExemple
* entry[+].fullUrl = "urn:uuid:257427f5-dffa-4a97-9475-4ebb988589af"
* entry[=].resource = Condition-Exemple-1
* entry[+].fullUrl = "urn:uuid:4d6a4b74-b2a7-4ec1-8db1-8925f435a916"
* entry[=].resource = Condition-Exemple-2
* entry[+].fullUrl = "urn:uuid:af0e13e5-d0a2-4dbc-b892-7328eca72ff4"
* entry[=].resource = Condition-Exemple-3
* entry[+].fullUrl = "urn:uuid:a4d49149-cda7-470d-a2e3-08f8d7db1bad"
* entry[=].resource = Condition-Exemple-4
* entry[+].fullUrl = "urn:uuid:803caba9-c128-4faa-94a0-d344738ebc63"
* entry[=].resource = Condition-Exemple-5
* entry[+].fullUrl = "urn:uuid:effd6c8a-6122-46cb-82ed-ef3f300ec17b"
* entry[=].resource = Condition-Exemple-6
* entry[+].fullUrl = "urn:uuid:18492799-4e37-47a7-b79b-18a4d0597963"
* entry[=].resource = Condition-Exemple-7
* entry[+].fullUrl = "urn:uuid:4e00844f-241b-47c0-85a4-9d612c46b3ad"
* entry[=].resource = Procedure-Exemple-1
* entry[+].fullUrl = "urn:uuid:5b0fcb3b-91fb-4870-bd97-8faa2794ea01"
* entry[=].resource = practitioner-role-exemple-2
* entry[+].fullUrl = "urn:uuid:da9eb2be-5993-4e39-827b-728fe3943a93"
* entry[=].resource = organizationExemple2
* entry[+].fullUrl = "urn:uuid:9e55af45-e661-472f-b713-4aaa2cfd1a77"
* entry[=].resource = practitioner-exemple-2
* entry[+].fullUrl = "urn:uuid:be247a4c-f8fe-4224-b51c-8c35d7124a2d"
* entry[=].resource = Condition-Exemple-8
* entry[+].fullUrl = "urn:uuid:3c6ec5d8-182d-4dfc-871b-59c9eb91aa51"
* entry[=].resource = Procedure-Exemple-2
* entry[+].fullUrl = "urn:uuid:0ccf6c51-d3f5-4817-bc24-7f63a2ef3c7f"
* entry[=].resource = AllergieIntolerance-Exemple-1
* entry[+].fullUrl = "urn:uuid:23f1c0a3-ce38-4817-9408-7feaeb04002d"
* entry[=].resource = EffetIndesirable-Exemple-1
* entry[+].fullUrl = "urn:uuid:0409af90-4717-4a93-9800-fcf546875dc8"
* entry[=].resource = MedicationAdministration-Exemple-1
* entry[+].fullUrl = "urn:uuid:f995f9bb-4043-45db-8b12-50dc7e9acc5a"
* entry[=].resource = Medication-Exemple-1
* entry[+].fullUrl = "urn:uuid:a4e1cafa-05e4-40c7-82dc-641f54281671"
* entry[=].resource = Condition-Exemple-9
* entry[+].fullUrl = "urn:uuid:1ff316e0-edde-4bb9-a5fe-822d486d8230"
* entry[=].resource = MedicationAdministration-Exemple-2
* entry[+].fullUrl = "urn:uuid:fb88edae-4d3f-4b00-ae17-71c8f03adc71"
* entry[=].resource = Medication-Exemple-2
* entry[+].fullUrl = "urn:uuid:034d19b3-3c4e-488a-a7c0-9181dfc721e3"
* entry[=].resource = DeviceUseStatement-Exemple-1
* entry[+].fullUrl = "urn:uuid:7bb3e1f4-15bd-4c06-a587-3445c0f5a530"
* entry[=].resource = Device-Exemple-1
* entry[+].fullUrl = "urn:uuid:41465f6b-d169-40b9-8d84-96064b4c3d28"
* entry[=].resource = DeviceUseStatement-Exemple-2
* entry[+].fullUrl = "urn:uuid:f38b1772-ca78-4578-be14-f7a493b2cbb9"
* entry[=].resource = Device-Exemple-2
* entry[+].fullUrl = "urn:uuid:960ebfbc-4b56-40b9-9990-cae5944d6e9b"
* entry[=].resource = Observation-Exemple1-Groupe-de-questionnaires-d-evaluation
* entry[+].fullUrl = "urn:uuid:e102b4b3-91b6-4b89-9065-560bc68c024c"
* entry[=].resource = Observation-Exemple-1-Evaluation
* entry[+].fullUrl = "urn:uuid:b9bca1ce-b850-408c-966c-26a1a87adf65"
* entry[=].resource = Observation-Exemple-2-Evaluation
* entry[+].fullUrl = "urn:uuid:ae1c9c50-9620-4755-a7a0-f72af5a82229"
* entry[=].resource = Observation-Exemple-3-Signe-vital-observe
* entry[+].fullUrl = "urn:uuid:44da5856-6555-4b43-b03f-176f45c29432"
* entry[=].resource = Observation-Exemple-4-Signe-vital-observe
* entry[+].fullUrl = "urn:uuid:48129cb5-0a81-4a17-aebd-8c6584b20cd4"
* entry[=].resource = Observation-Exemple-5-Habitus-Mode-De-Vie
* entry[+].fullUrl = "urn:uuid:3618f351-cea4-4834-8cf9-10151b74436b"
* entry[=].resource = Observation-Exemple-6-Habitus-Mode-De-Vie
* entry[+].fullUrl = "urn:uuid:4f0f3856-74a9-4a57-b173-e8342735d6c9"
* entry[=].resource = Observation-Exemple-7-Habitus-Mode-De-Vie
* entry[+].fullUrl = "urn:uuid:3799b968-d637-4a16-ad7b-5f79987dcdb1"
* entry[=].resource = Observation-Exemple-8-Habitus-Mode-De-Vie
* entry[+].fullUrl = "urn:uuid:541404fa-fc9c-4552-8d36-10adcc37f34e"
* entry[=].resource = FamilyMemberHistory-Exemple-1
* entry[+].fullUrl = "urn:uuid:7f0d585b-9a48-4605-baab-30e93603a563"
* entry[=].resource = Immunizations-Exemple-1
* entry[+].fullUrl = "urn:uuid:3a7417c1-d483-4f56-833d-43bf8e438874"
* entry[=].resource = Immunizations-Exemple-2
* entry[+].fullUrl = "urn:uuid:f3062170-ef7b-45a7-802f-47e461a7d05d"
* entry[=].resource = practitioner-role-exemple-3
* entry[+].fullUrl = "urn:uuid:4dbc2eff-6a94-4c7b-b49f-0f7a9790bef6"
* entry[=].resource = organization-Exemple-3
* entry[+].fullUrl = "urn:uuid:ab96b0ff-374a-45c7-82e4-aaa8888a2fec"
* entry[=].resource = practitioner-exemple-3
* entry[+].fullUrl = "urn:uuid:80c62f71-7c99-458d-884d-725bc410a14d"
* entry[=].resource = Condition-Exemple-10
* entry[+].fullUrl = "urn:uuid:bea6c387-6dca-4f55-93b7-7079e1c88dfb"
* entry[=].resource = Observation-Exemple-8-sur-la-grossesse
* entry[+].fullUrl = "urn:uuid:bdc0e3ae-128b-46de-b1a7-1c564cca6a61"
* entry[=].resource = Observation-Exemple-9-sur-la-grossesse
* entry[+].fullUrl = "urn:uuid:e4299828-2563-4265-9297-c27a8240b6ba"
* entry[=].resource = Observation-Exemple-10-sur-la-grossesse
* entry[+].fullUrl = "urn:uuid:091bf5bc-2ed8-4328-a5e6-7d78aeeeb20b"
* entry[=].resource = Observation-Exemple-11-sur-la-grossesse
* entry[+].fullUrl = "urn:uuid:73af1b58-f567-4c03-a1a6-d65d3b3fd079"
* entry[=].resource = MedicationAdministration-Exemple-3
* entry[+].fullUrl = "urn:uuid:83e3e384-84ca-46dd-aa8c-c847a70b1fb5"
* entry[=].resource = MedicationAdministration-Exemple-4
* entry[+].fullUrl = "urn:uuid:2f861880-31d2-4969-a2ce-b78f750f430e"
* entry[=].resource = Medication-Exemple-3
* entry[+].fullUrl = "urn:uuid:6349c4d2-bdba-41b1-b5d6-1ce8c359205d"
* entry[=].resource = Medication-Exemple-4

Instance: Composition-IPS-FR
InstanceOf: Composition
Usage: #inline
* meta.lastUpdated = "2025-09-09T13:28:17.000+00:00"
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-composition-document"
* language = #fr-FR
* extension.url = "http://hl7.org/fhir/5.0/StructureDefinition/extension-Composition.version"
* extension.valueString = "2"
* identifier.value = "1.2.250.1.213.1.1.1.51.2024.1"
* status = #final
* type = $loinc#60591-5 "Synthèse médicale"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* encounter = Reference(urn:uuid:51807e91-cb17-4ca1-bc58-1efa85cf9d72)
* date = "2025-09-09T14:00:00+01:00"
* author.extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-author-time"
* author.extension.valueDateTime = "2024-04-21T13:45:00+01:00"
* author = Reference(urn:uuid:a11d31c5-77ff-4642-91f7-66c4d10d18c9) "DR Stéphane MEDIONI"
* title = "SYNTHESE MEDICALE"
* confidentiality = #N
* attester.mode = #legal
* attester.time = "2024-01-04T15:00:00+01:00"
* attester.party = Reference(urn:uuid:a11d31c5-77ff-4642-91f7-66c4d10d18c9) "DR Stéphane MEDIONI"
* custodian = Reference(urn:uuid:579f1274-8265-4bb1-91ba-d093a11be4f5) "Centre de soins le Belvédère"
* relatesTo.code = #replaces
* relatesTo.targetIdentifier.type = $v2-0203#RI "Relates to Identifier"
* relatesTo.targetIdentifier.system = "urn:ietf:rfc:3986"
* relatesTo.targetIdentifier.value = "urn:uuid:8D5E778C-E155-4685-95C6-5FF65A362964"
* event.extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-performer-event"
* event.extension.valueReference = Reference(urn:uuid:a11d31c5-77ff-4642-91f7-66c4d10d18c9) "DR Stéphane MEDIONI"
* event.period.start = "2024-04-21T08:00:00+01:00"
* section[0].title = "Problèmes actifs"
* section[=].code = $loinc#11450-4 "Liste des problèmes actifs"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><table border=\"1\"><thead><tr><th>Date</th><th>Type</th><th>Problème</th><th>Sévérité</th><th>Statut du problème</th><th>Statut clinique du patient</th><th>Certitude</th><th>Commentaire</th><th>Document référencé</th></tr></thead><tbody><tr><td>2023-07-05</td><td>interprétation diagnostique</td><td>Thyroïdite auto-immune</td><td>modéré</td><td>Actif</td><td>en forme</td><td>Confirmé</td><td/><td/></tr><tr><td>2023-07-05</td><td>interprétation diagnostique</td><td>Diabète insulino-dépendant</td><td>modéré</td><td>Actif</td><td>en forme</td><td>Confirmé</td><td/><td/></tr><tr><td>2023-05-21</td><td>interprétation diagnostique</td><td>Épilepsie</td><td>modéré</td><td>Actif</td><td>en forme</td><td>Confirmé</td><td/><td/></tr><tr><td>2023-05-21</td><td>interprétation diagnostique</td><td>Angiœdème bradykinique</td><td>léger à modéré</td><td>Actif</td><td>asymptomatique</td><td>Confirmé</td><td/><td/></tr><tr><td>2023-05-21</td><td>interprétation diagnostique</td><td>Causes inconnues et non précisées de morbidité</td><td>léger</td><td>Récurrent</td><td>en forme</td><td>Non confirmé</td><td>texte libre</td><td/></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:257427f5-dffa-4a97-9475-4ebb988589af)
* section[=].entry[+] = Reference(urn:uuid:4d6a4b74-b2a7-4ec1-8db1-8925f435a916)
* section[=].entry[+] = Reference(urn:uuid:af0e13e5-d0a2-4dbc-b892-7328eca72ff4)
* section[=].entry[+] = Reference(urn:uuid:a4d49149-cda7-470d-a2e3-08f8d7db1bad)
* section[=].entry[+] = Reference(urn:uuid:803caba9-c128-4faa-94a0-d344738ebc63)
* section[+].title = "Antécédents"
* section[=].code = $loinc#11348-0 "Antécédents médicaux"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><table border=\"1\"><thead><tr><th>Date</th><th>Type</th><th>Problème</th><th>Sévérité</th><th>Statut du problème</th><th>Statut clinique du patient</th><th>Certitude</th><th>Commentaire</th></tr></thead><tbody><tr><td>2018-05-21</td><td>interprétation diagnostique</td><td>Accident Ischémique Cérébral Transitoire</td><td>modéré</td><td>Résolu</td><td>en forme</td><td>Non confirmé</td><td>texte libre</td></tr><tr><td>2018-05-21</td><td>interprétation diagnostique</td><td>Pyélonéphrite aiguë</td><td>modéré</td><td>Résolu</td><td>en forme</td><td>Non confirmé</td><td>texte libre</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:effd6c8a-6122-46cb-82ed-ef3f300ec17b)
* section[=].entry[+] = Reference(urn:uuid:18492799-4e37-47a7-b79b-18a4d0597963)
* section[+].title = "Historique des actes"
* section[=].code = $loinc#47519-4 "Historique des actes"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><table border=\"0\"><thead><tr><th>Date</th><th>Acte</th><th>Voie d'abord</th><th>Localisation anatomique</th><th>Motif</th><th>Commentaire</th></tr></thead><tbody><tr><td>14/01/2018</td><td>Dilatation intraluminale de 2 vaisseaux coronaires avec pose d'endoprothèse, par voie artérielle transcutanée</td><td>Voie intraartérielle</td><td>artère coronaire</td><td>Infarctus inférieur transmural</td><td>(texte libre)</td></tr><tr><td>08/12/2016</td><td>Autre acte (texte libre)</td><td>-</td><td>-</td><td>-</td><td>(texte libre)</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:4e00844f-241b-47c0-85a4-9d612c46b3ad)
* section[=].entry[+] = Reference(urn:uuid:3c6ec5d8-182d-4dfc-871b-59c9eb91aa51)
* section[+].title = "Allergies, hypersensibilités non allergiques, intolérances, idiosyncrasie"
* section[=].code = $loinc#48765-2 "Allergies et hypersensibilités"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><table border=\"0\"><thead><tr><th>Date</th><th>Type</th><th>Agent responsable</th><th>Statut</th><th>Criticité</th><th>Certitude</th><th>Réaction(s)</th><th>Sévérité réaction(s)</th><th>Commentaire</th></tr></thead><tbody><tr><td>04/12/2021</td><td>Allergie médicamenteuse</td><td>Paracétamol</td><td>Actif</td><td>Bas</td><td>Confirmé</td><td>Bronchospasme d'origine médicamenteuse<br/>Troubles allergiques ou d'hypersensibilité de la peau ou des muqueuses</td><td>modéré<br/>léger</td><td>(texte libre)</td></tr></tbody></table></div>"
* section[=].entry = Reference(urn:uuid:0ccf6c51-d3f5-4817-bc24-7f63a2ef3c7f)
* section[+].title = "Effets indésirables prévisibles liés aux médicaments"
* section[=].code = $loinc#44939-7 "Effets indésirables prévisibles liés aux médicaments"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><table border=\"0\"><thead><tr><th>Date</th><th>Type</th><th>Agent responsable</th><th>Substance(s) incriminée(s)</th><th>Posologie</th><th>Voie d’administration</th><th>Réaction(s)</th><th>Imputabilité</th><th>Gravité</th><th>Evolution</th><th>Commentaire</th></tr></thead><tbody><tr><td>04/12/2021</td><td>interaction médicamenteuse</td><td>AMOXICILLINE EG 1G BUV SACH 6</td><td>AMOXICILLINE TRIHYDRATÉE</td><td>1 g / 2 fois par jour</td><td>voie orale</td><td>nausées</td><td>probable</td><td>Non grave</td><td>Guérison sans séquelle</td><td>(texte libre)</td></tr></tbody></table></div>"
* section[=].entry = Reference(urn:uuid:23f1c0a3-ce38-4817-9408-7feaeb04002d)
* section[+].title = "Traitements"
* section[=].code = $loinc#10160-0 "Historique de la prise médicamenteuse"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><p><em>Traitements au long cours :</em></p><table border=\"0\"><thead><tr><th>Date de début</th><th>Date de fin</th><th>Médicament</th><th>Fréquence</th><th>Dose prescrite</th><th>Rythme</th><th>Voie d'administration</th><th>Motif</th><th>Commentaire</th></tr></thead><tbody><tr><td>11/01/2024</td><td>–</td><td>LEVOTHYROX 75 microgrammes, comprimé sécable</td><td>1 fois/j</td><td>1 cp</td><td>–</td><td>Voie orale</td><td>Thyroïdite auto-immune</td><td>Le patient a pris le médicament après le petit-déjeuner</td></tr></tbody></table></div>"
* section[=].entry = Reference(urn:uuid:1ff316e0-edde-4bb9-a5fe-822d486d8230)
* section[+].title = "Dispositifs médicaux"
* section[=].code = $loinc#46264-8 "Dispositifs médicaux"
* section[=].text.status = #generated
* section[=].text.div = "<div><table border=\"0\"><thead><tr><th>Date début</th><th>Date fin</th><th>Type de DM</th><th>ID du DM</th><th>Commentaire</th></tr></thead><tbody><tr><td>11/08/2019</td><td/><td>STIMULATEUR CARDIAQUE IMPLANTABLE TRIPLE CHAMBRE</td><td>inconnu</td><td>Stimulateur cardiaque contrôlé et fonctionnel</td></tr><tr><td>11/08/2013</td><td/><td>Autre DM : (texte libre)</td><td>inconnu</td><td>(texte libre)</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:034d19b3-3c4e-488a-a7c0-9181dfc721e3)
* section[=].entry[+] = Reference(urn:uuid:f38b1772-ca78-4578-be14-f7a493b2cbb9)
* section[+].title = "Points de vigilance"
* section[=].code = $loinc#44944-7 "Autres alertes"
* section[=].text.status = #generated
* section[=].text.div = "<div><p>Surveiller tension artérielle</p></div>"
* section[+].title = "Statut fonctionnel"
* section[=].code = $loinc#47420-5 "Évaluation du statut fonctionnel"
* section[=].text.status = #generated
* section[=].text.div = "<div><table border=\"0\"><thead><tr><th>Date</th><th>Type</th><th>Observation</th><th>Commentaire</th></tr></thead><tbody><tr><td>14/01/2018</td><td>Score de performance ECOG</td><td>Capable d’une activité identique à celle précédant la maladie sans aucune restriction (LOINC : LA9622-7)</td><td>(texte libre)</td></tr><tr><td>14/01/2018</td><td>Marcher</td><td>Restriction modérée de la performance de marche sur de courtes distances (CIF : d4500.3)</td><td>(texte libre)</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:960ebfbc-4b56-40b9-9990-cae5944d6e9b)
* section[=].entry[+] = Reference(urn:uuid:b9bca1ce-b850-408c-966c-26a1a87adf65)
* section[+].title = "Constantes"
* section[=].code = $loinc#8716-3 "Signes vitaux"
* section[=].text.status = #generated
* section[=].text.div = "<div><table border=\"0\"><thead><tr><th>Signe vital</th><th>Valeur</th><th>Date de la mesure</th><th>Commentaire</th></tr></thead><tbody><tr><td>Poids</td><td>58 kg</td><td>02/04/2024</td><td>(texte libre)</td></tr><tr><td>Taille</td><td>1,60 m</td><td>02/04/2024</td><td>(texte libre)</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:ae1c9c50-9620-4755-a7a0-f72af5a82229)
* section[=].entry[+] = Reference(urn:uuid:44da5856-6555-4b43-b03f-176f45c29432)
* section[+].title = "Mode de vie"
* section[=].code = $loinc#29762-2 "Habitus, Mode de vie"
* section[=].text.status = #generated
* section[=].text.div = "<div><table border=\"0\"><thead><tr><th>Date</th><th>Type</th><th>Observation</th><th>Commentaire</th></tr></thead><tbody><tr><td>non renseignée</td><td>Statut tabagique</td><td>Fumeur quotidien</td><td>(Texte libre)</td></tr><tr><td>non renseignée</td><td>Consommation tabagique</td><td>25 PA</td><td>(Texte libre)</td></tr><tr><td>non renseignée</td><td>Consommation d'alcool</td><td>5 verres / jour</td><td>(Texte libre)</td></tr><tr><td>non renseignée</td><td>Consommation de drogue</td><td>Cannabis</td><td>(Texte libre)</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:48129cb5-0a81-4a17-aebd-8c6584b20cd4)
* section[=].entry[+] = Reference(urn:uuid:3618f351-cea4-4834-8cf9-10151b74436b)
* section[=].entry[+] = Reference(urn:uuid:4f0f3856-74a9-4a57-b173-e8342735d6c9)
* section[=].entry[+] = Reference(urn:uuid:3799b968-d637-4a16-ad7b-5f79987dcdb1)
* section[+].title = "Facteurs de risques professionnels"
* section[=].code = $loinc#10161-8 "Facteurs de risques professionnels"
* section[=].text.status = #generated
* section[=].text.div = "<div><p>Contact répété avec solvants organiques (atelier peinture)</p></div>"
* section[+].title = "Facteurs de risques professionnels"
* section[=].code = $loinc#10157-6 "Historique des pathologies familiales"
* section[=].text.status = #generated
* section[=].text.div = "<div><table border=\"0\"><thead><tr><th>Lien de parenté</th><th>Antécédent</th><th>Commentaire</th></tr></thead><tbody><tr><td>Mère</td><td>Anémie à hématies falciformes sans crises (CIM-10 : D57.1)</td><td>(texte libre)</td></tr></tbody></table></div>"
* section[=].entry = Reference(urn:uuid:541404fa-fc9c-4552-8d36-10adcc37f34e)
* section[+].title = "Vaccinations"
* section[=].code = $loinc#11369-6 "Historique des vaccinations"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><table border=\"0\"><thead><tr><th>Date</th><th>Maladie(s) prévenue(s)</th><th>Vaccin</th><th>Lot n°</th><th>Type / Rang</th><th>Voie</th><th>Région d'administration</th><th>Réaction observée</th><th>Vaccinateur</th><th>Commentaire</th></tr></thead><tbody><tr><td>28/09/2009</td><td>Diphtérie-poliomyélite-tétanos</td><td>REVAXIS, suspension injectable en seringue préremplie. Vaccin diphtérique, tétanique et poliomyélitique (inactivé), adsorbé, à teneur réduite en antigènes</td><td>4456672</td><td>1ère série vaccinante / 2</td><td>Voie intramusculaire</td><td>Deltoïde gauche</td><td>Fièvre due à des médicaments</td><td>Dr Charles MULLER</td><td>Prise récente et ponctuelle de solupred (60mg) en une prise pendant 2 jours</td></tr><tr><td>25/08/2009</td><td>Diphtérie-poliomyélite-tétanos</td><td>REVAXIS, suspension injectable en seringue préremplie. Vaccin diphtérique, tétanique et poliomyélitique (inactivé), adsorbé, à teneur réduite en antigènes</td><td>4456668</td><td>1ère série vaccinante / 1</td><td>Voie intramusculaire</td><td>Deltoïde gauche</td><td>Fièvre due à des médicaments</td><td>Dr Charles MULLER</td><td>-</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:7f0d585b-9a48-4605-baab-30e93603a563)
* section[=].entry[+] = Reference(urn:uuid:3a7417c1-d483-4f56-833d-43bf8e438874)
* section[+].title = "Historique des grossesses"
* section[=].code = $loinc#10162-6 "Historique des grossesses"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><table border=\"0\"><thead><tr><th>Observation</th><th>Précision</th></tr></thead><tbody><tr><td>Statut de grossesse</td><td>patiente actuellement enceinte</td></tr><tr><td>Date prévisionnelle d'accouchement</td><td>03/09/2024</td></tr><tr><td>Nb d'enfants vivants</td><td>1</td></tr><tr><td>Nombre d'interruptions de grossesse</td><td>1</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:bea6c387-6dca-4f55-93b7-7079e1c88dfb)
* section[=].entry[+] = Reference(urn:uuid:bdc0e3ae-128b-46de-b1a7-1c564cca6a61)
* section[=].entry[+] = Reference(urn:uuid:e4299828-2563-4265-9297-c27a8240b6ba)
* section[=].entry[+] = Reference(urn:uuid:091bf5bc-2ed8-4328-a5e6-7d78aeeeb20b)
* section[+].title = "Plan de soins"
* section[=].code = $loinc#18776-5 "Plan de soins"
* section[=].text.status = #generated
* section[=].text.div = "<div xmlns=\"http://www.w3.org/1999/xhtml\"><p><em>Traitements médicamenteux :</em></p><table border=\"0\"><thead><tr><th>Médicament</th><th>Commentaire</th></tr></thead><tbody><tr><td>ROSUVASTATINE EG 5 mg, comprimé pelliculé</td><td>(texte libre)</td></tr><tr><td>Autre traitement : (texte libre)</td><td>(texte libre)</td></tr></tbody></table><br/><p><em>Demandes d'examens ou de suivis :</em></p><table border=\"0\"><thead><tr><th>Date envisagée</th><th>Demande</th><th>Commentaire</th></tr></thead><tbody><tr><td>01/12/2024</td><td>Cholestérol LDL</td><td>(Texte libre)</td></tr><tr><td>01/12/2024</td><td>Autre examen (texte libre)</td><td>(Texte libre)</td></tr></tbody></table><br/><p><em>Actes :</em></p><table border=\"0\"><thead><tr><th>Date envisagée</th><th>Priorité</th><th>Acte</th><th>Commentaire</th></tr></thead><tbody><tr><td>01/12/2024</td><td>Bénéfique pour le patient mais pas essentiel pour sa survie</td><td>Pose d'une prothèse auditive implantable dans l'oreille moyenne</td><td>(Texte libre)</td></tr><tr><td>01/12/2024</td><td>Aussi vite que possible</td><td>Autre acte (texte libre)</td><td>(Texte libre)</td></tr></tbody></table><br/><p><em>Rencontres :</em></p><table border=\"0\"><thead><tr><th>Date envisagée</th><th>Type de rencontre</th><th>Personne</th><th>Lieu</th><th>Commentaire</th></tr></thead><tbody><tr><td>01/12/2024</td><td>Ambulatoire</td><td>PR Jacques PETITJEAN (Médecin - Oncologie, opt Onco-hématologie (SM))</td><td>Hôpital Lariboisière (Service hématologie)</td><td>(Texte libre)</td></tr></tbody></table><br/><p><em>Vaccins recommandés :</em></p><table border=\"0\"><thead><tr><th>Période de vaccination souhaitable</th><th>Vaccin</th><th>Commentaire</th></tr></thead><tbody><tr><td>entre le 01/12/2024 et le 31/12/2024</td><td>Vaccin antituberculeux</td><td>(Texte libre)</td></tr><tr><td>entre le 01/12/2024 et le 31/12/2024</td><td>Autre vaccin (texte libre)</td><td>(Texte libre)</td></tr></tbody></table></div>"
* section[=].entry[0] = Reference(urn:uuid:73af1b58-f567-4c03-a1a6-d65d3b3fd079)
* section[=].entry[+] = Reference(urn:uuid:83e3e384-84ca-46dd-aa8c-c847a70b1fb5)

Instance: patientExemple
InstanceOf: Patient
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-patient-ins-document"
* extension[0].extension.url = "identityStatus"
* extension[=].extension.valueCoding = $fr-core-cs-v2-0445#VALI
* extension[=].url = "https://hl7.fr/ig/fhir/core/StructureDefinition/fr-core-identity-reliability"
* extension[+].url = "http://hl7.org/fhir/StructureDefinition/patient-birthPlace"
* extension[=].valueAddress.extension.url = "https://hl7.fr/ig/fhir/core/StructureDefinition/fr-core-address-insee-code"
* extension[=].valueAddress.extension.valueCoding = $TRE-R13-Commune#51215
* extension[=].valueAddress.city = "DOMPREMY"
* identifier.use = #official
* identifier.type = $fr-core-cs-v2-0203#INS-NIR-TEST
* identifier.system = "urn:oid:1.2.250.1.213.1.4.10"
* identifier.value = "279035121518989"
* name[0].extension.url = "https://hl7.fr/ig/fhir/core/StructureDefinition/fr-core-patient-birth-list-given-name"
* name[=].extension.valueString = "DOMINIQUE MARIE-LOUISE"
* name[=].use = #official
* name[=].family = "PAT-TROIS"
* name[=].given = "DOMINIQUE MARIE-LOUISE"
* name[+].use = #usual
* name[=].family = "PAT-TROIS"
* name[=].given = "DOMINIQUE"
* telecom[0].system = #phone
* telecom[=].value = "0144534551"
* telecom[=].use = #home
* telecom[+].system = #phone
* telecom[=].value = "0647151010"
* telecom[=].use = #work
* telecom[=].rank = 1
* telecom[+].system = #email
* telecom[=].value = "279035121518989@patient.mssante.fr"
* telecom[=].use = #home
* gender = #female
* birthDate = "1979-03-28"
* deceasedBoolean = false
* address.use = #home
* address.type = #physical
* address.line[0] = "28"
* address.line[+] = "Avenue de Breteuil"
* address.line[+] = "Escalier A"
* address.city = "Paris"
* address.postalCode = "75007"
* contact[0].relationship = $fr-core-vs-patient-contact-role#GUARD "Responsable légal"
* contact[=].name.family = "NESSI"
* contact[=].name.given = "Jeanne"
* contact[=].name.prefix = "MME"
* contact[=].address.use = #home
* contact[=].address.type = #physical
* contact[=].address.line = "28 Avenue de Breteuil"
* contact[=].address.city = "Paris"
* contact[+].relationship[0] = $JDV-J11-RelationPatient-CISIS#SIS "Soeur"
* contact[=].relationship[+] = $fr-core-vs-patient-contact-role#ECON "Personne à prévenir en cas d'urgence"
* contact[=].name.text = "Sophie NESSI"
* contact[=].name.family = "NESSI"
* contact[=].address.text = "12 rue des Lilas, 75012 Paris"
* contact[+].relationship[0] = $JDV-J11-RelationPatient-CISIS#SIS "Soeur"
* contact[=].relationship[+] = $jdv-role-informateur-cisis#NOK "Personne de confiance"
* contact[=].name.text = "Sophie NESSI"
* contact[=].name.family = "NESSI"
* contact[=].address.text = "12 rue des Lilas, 75012 Paris"
* generalPractitioner = Reference(urn:uuid:a11d31c5-77ff-4642-91f7-66c4d10d18c9) "DR Stéphane MEDIONI"

Instance: practitioner-role-exemple-1
InstanceOf: PractitionerRole
Usage: #inline
* meta.lastUpdated = "2013-05-05T16:13:03Z"
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-practitionerRole-document"
* practitioner = Reference(urn:uuid:b5941194-08be-4893-a629-652f97587b39)
* organization = Reference(urn:uuid:579f1274-8265-4bb1-91ba-d093a11be4f5)
* code = $TRE-R259-HL7ParticipationFunction#PCP "Médecin traitant"

Instance: practitioner-exemple-1
InstanceOf: Practitioner
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-practitioner-document"
* identifier.type = $fr-core-cs-v2-0203#RPPS "Numéro du professionnel de santé"
* identifier.system = "https://rpps.esante.gouv.fr"
* identifier.value = "801234567897"
* name.family = "MEDIONI"
* name.given = "Stéphane"
* name.suffix = "DR"
* telecom[0].system = #phone
* telecom[=].value = "0147150000"
* telecom[=].use = #work
* telecom[=].rank = 1
* telecom[+].system = #email
* telecom[=].value = "mailto:stephane.medioni@mssante.fr"
* telecom[=].use = #home
* address.text = "8 Rue Petit Pont Paris 75005"
* qualification.code = $TRE-R38-SpecialiteOrdinale#SM26 "Qualifié en Médecine générale (SM)"

Instance: organizationExemple
InstanceOf: Organization
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-organization-document"
* identifier.system = "urn:oid:1.2.250.1.71.4.2.2"
* identifier.value = "2801234567"
* type = $JDV-J04-XdsPracticeSettingCode-CISIS#ETABLISSEMENT "Etablissement de santé"
* name = "Centre de soins le Belvédère"
* address.line = "3 Rue Petit Pont"
* address.city = "PARIS"
* address.postalCode = "75005"

Instance: encounterExemple
InstanceOf: Encounter
Usage: #inline
* meta.lastUpdated = "2024-05-07T11:28:17.000+00:00"
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-encounter-document"
* identifier.type = $fr-core-vs-encounter-identifier-type#VN "Visit Number"
* identifier.system = "urn:uuid:1.2.250.1.71.4.2.1"
* identifier.value = "801234534765"
* status = #finished
* class = $v3-ActCode#AMB "ambulatoire (hors établissement)"
* period.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* period.extension.valueCode = #not-applicable
* location.location = Reference(urn:uuid:648170b7-1538-44a7-8820-df04dcc32fad)

Instance: locationExemple
InstanceOf: Location
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-location-document"
* type = $JDV-J02-XdsHealthcareFacilityTypeCode-CISIS#SA04 "Etablissement privé non PSPH"

Instance: Condition-Exemple-1
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition-clinical-status-extension"
* extension.valueCodeableConcept = $sct#102499006 "en forme"
* identifier.system = "urn:uuid"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1552472"
* clinicalStatus = #active "Actif"
* verificationStatus = $condition-ver-status#confirmed "Confirmé"
* category = $sct#282291009 "interprétation diagnostique"
* severity = $jdv-severite-observation-cisis#6736007 "modéré"
* code = $terminologie-cim-10#E06.3 "Thyroïdite auto-immune"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2024-01-11"

Instance: Condition-Exemple-2
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition-clinical-status-extension"
* extension.valueCodeableConcept = $sct#102499006 "en forme"
* identifier.system = "urn:uuid"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1552472"
* clinicalStatus = #active "Actif"
* verificationStatus = $condition-ver-status#confirmed "Confirmé"
* category = $sct#282291009 "interprétation diagnostique"
* severity = $jdv-severite-observation-cisis#6736007 "modéré"
* code = $terminologie-cisp#T89 "Diabète insulino-dépendant"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2023-07-05"

Instance: Condition-Exemple-3
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition-clinical-status-extension"
* extension.valueCodeableConcept = $sct#102499006 "en forme"
* identifier.system = "urn:uuid"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1552472"
* clinicalStatus = #active "Actif"
* verificationStatus = $condition-ver-status#confirmed "Confirmé"
* category = $sct#282291009 "interprétation diagnostique"
* severity = $jdv-severite-observation-cisis#6736007 "modéré"
* code = #114 "epilepsie"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2023-05-21"

Instance: Condition-Exemple-4
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition-clinical-status-extension"
* extension.valueCodeableConcept = $sct#84387000 "asymptomatique"
* identifier.system = "urn:uuid"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1552472"
* clinicalStatus = #active "Actif"
* verificationStatus = $condition-ver-status#confirmed "Confirmé"
* category = $sct#282291009 "interprétation diagnostique"
* severity = $jdv-severite-observation-cisis#371923003 "léger à modéré"
* code = #658 "Angiœdème bradykinique"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2023-05-21"

Instance: Condition-Exemple-5
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition-clinical-status-extension"
* extension.valueCodeableConcept = $sct#102499006 "en forme"
* identifier.system = "urn:uuid"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1552472"
* clinicalStatus = #recurrence "Récurrent"
* verificationStatus = $condition-ver-status#unconfirmed "Non confirmé"
* category = $sct#282291009 "interprétation diagnostique"
* severity = $jdv-severite-observation-cisis#255604002 "léger"
* code = $terminologie-cim-10#R69 "Causes inconnues et non précisées de morbidité"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2023-05-21"
* note.text = "texte libre"

Instance: Condition-Exemple-6
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition-clinical-status-extension"
* extension.valueCodeableConcept = $sct#102499006 "en forme"
* identifier.system = "urn:uuid"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1552472"
* clinicalStatus = #resolved "Résolu"
* verificationStatus = $condition-ver-status#unconfirmed "Non confirmé"
* category = $sct#282291009 "interprétation diagnostique"
* severity = $jdv-severite-observation-cisis#6736007 "modéré"
* code = $terminologie-cim-10#G45.9 "Accident Ischémique Cérébral Transitoire"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2018-05-21"
* note.text = "texte libre"

Instance: Condition-Exemple-7
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition-clinical-status-extension"
* extension.valueCodeableConcept = $sct#102499006 "en forme"
* identifier.system = "urn:uuid"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1552472"
* clinicalStatus = #resolved "Résolu"
* verificationStatus = $condition-ver-status#unconfirmed "Non confirmé"
* category = $sct#282291009 "interprétation diagnostique"
* severity = $jdv-severite-observation-cisis#6736007 "modéré"
* code = $terminologie-cim-10#N10 "Pyélonéphrite aiguë"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2018-05-21"
* note.text = "texte libre"

Instance: Procedure-Exemple-1
InstanceOf: Procedure
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-acte"
* identifier.system = "urn:uuid"
* identifier.value = "A6BC7FD2-EC3F-4E01-B567-854B087D1D9B"
* status = #completed
* code = $terminologie-ccam#DDAF004 "Dilatation intraluminale de 2 vaisseaux coronaires avec pose d'endoprothèse, par voie artérielle transcutanée"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* performedDateTime = "2018-01-14"
* recorder.extension.extension[0].url = "actor"
* recorder.extension.extension[=].valueReference = Reference(urn:uuid:5b0fcb3b-91fb-4870-bd97-8faa2794ea01)
* recorder.extension.extension[+].url = "type"
* recorder.extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-actor-extension"
* reasonReference = Reference(urn:uuid:be247a4c-f8fe-4224-b51c-8c35d7124a2d)
* bodySite[0] = $sct#229801003 "intra-arterial"
* bodySite[+] = $sct#41801008 "artère coronaire"
* note.text = "texte libre"

Instance: practitioner-role-exemple-2
InstanceOf: PractitionerRole
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-practitionerRole-document"
* practitioner = Reference(urn:uuid:9e55af45-e661-472f-b713-4aaa2cfd1a77)
* organization = Reference(urn:uuid:da9eb2be-5993-4e39-827b-728fe3943a93)

Instance: organizationExemple2
InstanceOf: Organization
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-organization-document"
* identifier.system = "urn:oid:1.2.250.1.71.4.2.2"
* identifier.value = "2801234567"
* name = "Hôpital Lariboisière"

Instance: practitioner-exemple-2
InstanceOf: Practitioner
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-practitioner-document"
* identifier.type = $fr-core-cs-v2-0203#RPPS "Numéro du professionnel de santé"
* identifier.system = "https://rpps.esante.gouv.fr"
* identifier.value = "801234567897"
* name.family = "PETITJEAN"
* name.given = "Jacques"
* name.prefix = "M"
* name.suffix = "PR"

Instance: Condition-Exemple-8
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* identifier.system = "urn:uuid"
* identifier.value = "FC21DC59-43D5-4BB0-ACC7-3601784BFBC0"
* clinicalStatus = #resolved "Résolu"
* category = $sct#282291009 "interprétation diagnostique"
* code = $terminologie-cim-10#N10 "Infarctus transmural inférieur"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2018-05-21"

Instance: Procedure-Exemple-2
InstanceOf: Procedure
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-acte"
* identifier.system = "urn:uuid"
* identifier.value = "A6BC7FD2-EC3F-4E01-B567-854B087D1D9B"
* status = #completed
* code = $terminologie-cisis#GEN-092.04.13 "Autre acte"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* performedDateTime = "2016-12-08"
* bodySite = $sct#229801003 "intra-arterial"
* note.text = "texte libre"

Instance: AllergieIntolerance-Exemple-1
InstanceOf: AllergyIntolerance
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-allergie-intolerance"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "C64C6013-C8AF-4938-AD10-1B7D26DEE2A0"
* clinicalStatus.coding.code = #active
* verificationStatus.coding.code = #confirmed
* type = #allergy
* category = #medication
* criticality = #low
* code = $sct#416098002 "allergie médicamenteuse"
* patient = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetPeriod.start = "2021-12-04"
* reaction[0].substance = $terminologie-cim11-mms#XM5DJ7 "Paracétamol"
* reaction[=].manifestation = $terminologie-cim11-mms#4A80.0 "Bronchospasme d'origine médicamenteuse"
* reaction[=].onset = "2021-12-04"
* reaction[=].severity = #moderate
* reaction[+].substance = $terminologie-cim11-mms#XM5DJ7 "Paracétamol"
* reaction[=].manifestation = $terminologie-cim11-mms#4A82 "Troubles allergiques ou d'hypersensibilité de la peau ou des muqueuses"
* reaction[=].onset = "2021-12-04"
* reaction[=].severity = #mild
* reaction[=].note.text = "texte libre"

Instance: EffetIndesirable-Exemple-1
InstanceOf: AdverseEvent
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-effet-indesirable"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "C64C6013-C8AF-4938-AD10-1B7D26DEE2A0"
* actuality = #actual
* category = $sct#79899007 "interaction médicamenteuse"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* date = "2021-12-04"
* detected = "2021-12-04"
* resultingCondition = Reference(urn:uuid:a4e1cafa-05e4-40c7-82dc-641f54281671)
* seriousness = $sct#255604002 "gravité légère"
* outcome = $sct#1352007006 "Guérison sans séquelle"
* suspectEntity.instance = Reference(urn:uuid:0409af90-4717-4a93-9800-fcf546875dc8)
* suspectEntity.causality.assessment = $sct#17162000 "certain(e)"

Instance: MedicationAdministration-Exemple-1
InstanceOf: MedicationAdministration
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-administration-document"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-administration-frequency"
* extension.valueTiming.repeat.frequency = 1
* extension.valueTiming.repeat.period = 12
* extension.valueTiming.repeat.periodUnit = #h
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "55915ED6-0682-4588-9F4A-FB71210F7015"
* status = #completed
* category = $v3-ActCode#DRUG "Médicament"
* medicationReference = Reference(urn:uuid:f995f9bb-4043-45db-8b12-50dc7e9acc5a)
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectivePeriod.start = "2021-12-04"
* dosage.route = #20053000 "Voie orale"

Instance: Medication-Exemple-1
InstanceOf: Medication
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-document"
* extension[0].url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-productname"
* extension[=].valueString = "AMOXICILLINE EG 1G BUV SACH 6"
* extension[+].url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-classification"
* extension[=].valueCodeableConcept = $terminologie-atc#J01CA04 "amoxicilline"
* extension[=].valueCodeableConcept.text = "amoxicilline"
* code = $terminologie-atc#3400935186607 "AMOXICILLINE EG 1G BUV SACH 6"
* ingredient.itemCodeableConcept.extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-nom-substance"
* ingredient.itemCodeableConcept.extension.valueString = "AMOXICILLINE TRIHYDRATÉE"
* ingredient.itemCodeableConcept = $terminologie-sms#100000092629 "AMOXICILLINE TRIHYDRATÉE"
* ingredient.strength.numerator.value = 1.148
* ingredient.strength.numerator.unit = "g"
* ingredient.strength.denominator.value = 1

Instance: Condition-Exemple-9
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* identifier.system = "urn:uuid"
* identifier.value = "C64C6013-C8AF-4938-AD10-1B7D26DEE5F7"
* clinicalStatus = #resolved "Résolu"
* category = $sct#418799008 "symptôme rapporté par le patient ou le répondant"
* code = $terminologie-cim-10#MD90.0 "Nausée"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2018-05-21"

Instance: MedicationAdministration-Exemple-2
InstanceOf: MedicationAdministration
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-administration-document"
* extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-administration-frequency"
* extension.valueTiming.repeat.frequency = 1
* extension.valueTiming.repeat.periodUnit = #d
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "55915ED6-0682-4588-9F4A-FB71210F7015"
* status = #completed
* category = $v3-ActCode#DRUG "Médicament"
* medicationReference = Reference(urn:uuid:fb88edae-4d3f-4b00-ae17-71c8f03adc71)
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectivePeriod.start = "2021-12-04"
* reasonReference = Reference(urn:uuid:257427f5-dffa-4a97-9475-4ebb988589af)
* note.text = "Le patient a pris le médicament après le petit-déjeuner"
* dosage.route = #20053000 "Voie orale"
* dosage.dose = 1 '{tbl}' "tablet"
* dosage.rateQuantity.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* dosage.rateQuantity.extension.valueCode = #not-applicable

Instance: Medication-Exemple-2
InstanceOf: Medication
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-document"
* extension[0].url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-productname"
* extension[=].valueString = "LEVOTHYROX 75 microgrammes"
* extension[+].url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-classification"
* extension[=].valueCodeableConcept = $terminologie-atc#H03AA01 "lévothyroxine sodique"
* extension[=].valueCodeableConcept.text = "lévothyroxine sodique"
* code = $terminologie-bdpm#3400930065785 "LEVOTHYROX 75 microgrammes, comprimé sécable plaquette(s) PVC aluminium de 30 comprimé(s)"
* ingredient.itemCodeableConcept.extension.url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-nom-substance"
* ingredient.itemCodeableConcept.extension.valueString = "paracetamol"
* ingredient.itemCodeableConcept = $terminologie-sms#100000091647 "LÉVOTHYROXINE SODIQUE"
* ingredient.strength.numerator.value = 75
* ingredient.strength.numerator.unit = "mg"
* ingredient.strength.denominator.value = 1

Instance: DeviceUseStatement-Exemple-1
InstanceOf: DeviceUseStatement
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-device-use-statement-medicaldevice"
* identifier.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* identifier.extension.valueCode = #unknown
* status = #active
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* timingPeriod.start = "2019-08-11"
* device = Reference(urn:uuid:7bb3e1f4-15bd-4c06-a587-3445c0f5a530) "STIMULATEUR CARDIAQUE IMPLANTABLE TRIPLE CHAMBRE"
* note.text = "Stimulateur cardiaque contrôlé et fonctionnel"

Instance: Device-Exemple-1
InstanceOf: Device
Usage: #inline
* type = $terminologie-emdn#J010104 "STIMULATEUR CARDIAQUE IMPLANTABLE TRIPLE CHAMBRE"

Instance: DeviceUseStatement-Exemple-2
InstanceOf: DeviceUseStatement
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-device-use-statement-medicaldevice"
* identifier.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* identifier.extension.valueCode = #unknown
* status = #active
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* timingPeriod.start = "2019-08-11"
* device = Reference(urn:uuid:f38b1772-ca78-4578-be14-f7a493b2cbb9) "Autre dispositif médical"
* note.text = "Autre DM : (texte libre)"

Instance: Device-Exemple-2
InstanceOf: Device
Usage: #inline
* type = $terminologie-cisis#GEN-092.02.02 "Autre dispositif médical"

Instance: Observation-Exemple1-Groupe-de-questionnaires-d-evaluation
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-groupe-questionnaires-evaluation"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "12DA3A06-18E7-40B7-9397-1FA5B1555683"
* status = #final
* code.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* code.extension.valueCode = #not-applicable
* effectiveDateTime = "2018-01-14T15:06:00+00:00"
* hasMember = Reference(urn:uuid:e102b4b3-91b6-4b89-9065-560bc68c024c)

Instance: Observation-Exemple-1-Evaluation
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-evaluation"
* status = #final
* category.coding.code = #survey
* code = $loinc#89247-1 "Score de performance ECOG"
* effectiveDateTime = "2018-01-14T15:06:00+00:00"
* valueCodeableConcept = $loinc#LA9622-7 "Capable d’une activité identique à celle précédant la maladie sans aucune restriction"
* note.text = "Texte libre"

Instance: Observation-Exemple-2-Evaluation
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-evaluation"
* status = #final
* category.coding.code = #survey
* code = $icf-nl#d450 "Marcher"
* effectiveDateTime = "2018-01-14T15:06:00+00:00"
* valueCodeableConcept = $icf-nl#d4500.3 "Restriction modérée de la performance de marche sur de courtes distances"
* note.text = "Texte libre"

Instance: Observation-Exemple-3-Signe-vital-observe
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-signes-vital-Observe-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "8D40D39D-3574-496A-91EA-B7BE236ABD1A"
* status = #final
* category = $observation-category#vital-signs
* code = $loinc#29463-7 "Poids"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectiveDateTime = "2024-04-02T09:58:00+00:00"
* valueQuantity = 58 'kg' "kg"
* note.text = "Texte libre"

Instance: Observation-Exemple-4-Signe-vital-observe
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-signes-vital-Observe-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "8D40D39D-3574-496A-91EA-B7BE236ABD1A"
* status = #final
* category = $observation-category#vital-signs
* code = $loinc#8302-2 "Taille"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectiveDateTime = "2024-04-02T09:58:00+00:00"
* valueQuantity = 1.6 'm' "m"
* note.text = "Texte libre"

Instance: Observation-Exemple-5-Habitus-Mode-De-Vie
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-habitus-mode-de-vie"
* status = #final
* category = $observation-category#social-history
* code = $loinc#72166-2 "Statut tabagique"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueCodeableConcept = $loinc#LA18976-3 "Fumeur quotidien"
* note.text = "Texte libre"

Instance: Observation-Exemple-6-Habitus-Mode-De-Vie
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-habitus-mode-de-vie"
* status = #final
* category = $observation-category#social-history
* code = $loinc#74011-8 "Consommation tabagique"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueQuantity = 25 '{pack}/a' "pack"
* note.text = "Texte libre"

Instance: Observation-Exemple-7-Habitus-Mode-De-Vie
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-habitus-mode-de-vie"
* status = #final
* category = $observation-category#social-history
* code = $loinc#74013-4 "Consommation d'alcool"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueQuantity = 5 '{drink}/d' "drink"
* note.text = "Texte libre"

Instance: Observation-Exemple-8-Habitus-Mode-De-Vie
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-habitus-mode-de-vie"
* status = #final
* category = $observation-category#social-history
* code = $loinc#11343-1 "Consommation de drogue non médicales"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueCodeableConcept = $sct#398705004 "Cannabis"
* note.text = "Texte libre"

Instance: FamilyMemberHistory-Exemple-1
InstanceOf: FamilyMemberHistory
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-family-member-history"
* status = #completed
* patient = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* date.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* date.extension.valueCode = #not-applicable
* relationship = $v3-RoleCode#MTH "Mère"
* note.text = "Texte libre"
* condition.code = $sct#282291009 "interprétation diagnostique"
* condition.outcome = $terminologie-cim-10#D57.1 "Anémie à hématies falciformes sans crises"

Instance: Immunizations-Exemple-1
InstanceOf: Immunization
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-vaccination-document"
* extension[0].url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-vaccination-type-extension"
* extension[=].valueCodeableConcept = $v3-ActCode#INITIMMUNIZ "1ère série vaccinante"
* extension[+].url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-productname"
* extension[=].valueString.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* extension[=].valueString.extension.valueCode = #unknown
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "41DAC707-7C96-47B7-B600-AC63D8002142"
* status = #completed
* vaccineCode = $terminologie-atc#J07CA01 "Diphtérie-poliomyélite-tétanos"
* patient = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* occurrenceDateTime = "2009-09-28"
* lotNumber = "4456672"
* site = $sct#16217701000119102 "Deltoïde gauche"
* route = $terminologie-standardterms#20035000 "Voie intramusculaire"
* performer.actor.extension[0].extension[0].url = "type"
* performer.actor.extension[=].extension[=].valueCode = #PRF
* performer.actor.extension[=].extension[+].url = "actor"
* performer.actor.extension[=].extension[=].valueReference = Reference(urn:uuid:f3062170-ef7b-45a7-802f-47e461a7d05d)
* performer.actor.extension[=].url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-actor-extension"
* performer.actor.extension[+].extension[0].url = "type"
* performer.actor.extension[=].extension[+].url = "actor"
* performer.actor.extension[=].url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-actor-extension"
* note.text = "texte libre"
* reasonReference = Reference(urn:uuid:80c62f71-7c99-458d-884d-725bc410a14d")
* protocolApplied.series = "1"
* protocolApplied.doseNumberPositiveInt = 1

Instance: Immunizations-Exemple-2
InstanceOf: Immunization
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-vaccination-document"
* extension[0].url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-vaccination-type-extension"
* extension[=].valueCodeableConcept = $v3-ActCode#INITIMMUNIZ "1ère série vaccinante"
* extension[+].url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-productname"
* extension[=].valueString.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* extension[=].valueString.extension.valueCode = #unknown
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "41DAC707-7C96-47B7-B600-AC63D8002142"
* status = #completed
* vaccineCode = $terminologie-atc#J07CA01 "Diphtérie-poliomyélite-tétanos"
* patient = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* occurrenceDateTime = "2009-09-28"
* lotNumber = "4456668"
* site = $sct#16217701000119102 "Deltoïde gauche"
* route = $terminologie-standardterms#20035000 "Voie intramusculaire"
* performer.actor.extension[0].extension[0].url = "type"
* performer.actor.extension[=].extension[=].valueCode = #PRF
* performer.actor.extension[=].extension[+].url = "actor"
* performer.actor.extension[=].extension[=].valueReference = Reference(urn:uuid:f3062170-ef7b-45a7-802f-47e461a7d05d)
* performer.actor.extension[=].url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-actor-extension"
* performer.actor.extension[+].extension[0].url = "type"
* performer.actor.extension[=].extension[+].url = "actor"
* performer.actor.extension[=].url = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-actor-extension"
* note.text = "texte libre"
* reasonReference = Reference(urn:uuid:80c62f71-7c99-458d-884d-725bc410a14d")
* protocolApplied.series = "2"
* protocolApplied.doseNumberPositiveInt = 2

Instance: practitioner-role-exemple-3
InstanceOf: PractitionerRole
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-practitionerRole-document"
* practitioner = Reference(urn:uuid:ab96b0ff-374a-45c7-82e4-aaa8888a2fec)
* organization = Reference(urn:uuid:4dbc2eff-6a94-4c7b-b49f-0f7a9790bef6)

Instance: organization-Exemple-3
InstanceOf: Organization
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-organization-document"
* identifier.system = "urn:oid:1.2.250.1.71.4.2.2"
* identifier.value = "21750803447"
* type = $TRE-A01-CadreExercice#AMBULATOIRE "Ambulatoire"
* name = "Cabinet médical du Dr MULLER"

Instance: practitioner-exemple-3
InstanceOf: Practitioner
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-practitioner-document"
* identifier.type = $fr-core-cs-v2-0203#RPPS "Numéro du professionnel de santé"
* identifier.system = "https://rpps.esante.gouv.fr"
* identifier.value = "801234567897"
* name.family = "MULLER<"
* name.given = "Charles"
* name.prefix = "M"
* name.suffix = "DR"

Instance: Condition-Exemple-10
InstanceOf: Condition
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-condition"
* identifier.system = "urn:uuid"
* identifier.value = "F9E020E2-D219-43D4-8BF8-6B275E1F726D"
* category = $sct#418799008 "symptôme rapporté par le patient ou le répondant"
* code = $terminologie-cim-10#R50.2 "Fièvre due à des médicaments"
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* onsetDateTime = "2009-09-28"

Instance: Observation-Exemple-8-sur-la-grossesse
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-grossesse-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "4FD525FD-8712-40F9-BF31-EF9A25069C25"
* status = #final
* code = $loinc#11449-6 "Statut de grossesse"
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueCodeableConcept = $sct#77386006 "enceinte"

Instance: Observation-Exemple-9-sur-la-grossesse
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-grossesse-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "4FD525FD-8712-40F9-BF31-EF9A25069C32"
* status = #final
* code = $loinc#11778-8 "Date estimée de l'accouchement"
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueDateTime = "2024-09-03"

Instance: Observation-Exemple-10-sur-la-grossesse
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-grossesse-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "4FD525FD-8712-40F9-BF31-EF9A25069C65"
* status = #final
* code = $loinc#11638-4 "Nombre d'enfants vivants"
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueInteger = 1

Instance: Observation-Exemple-11-sur-la-grossesse
InstanceOf: Observation
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-observation-grossesse-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "4FD525FD-8712-40F9-BF31-EF9A25069C65"
* status = #final
* code = $loinc#11612-9 "Nombre d'interruptions de grossesse"
* effectiveDateTime.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* effectiveDateTime.extension.valueCode = #not-applicable
* valueInteger = 1

Instance: MedicationAdministration-Exemple-3
InstanceOf: MedicationAdministration
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-administration-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "AADC9C14-F1CA-4177-B2C8-A5178D5B3CA0"
* status = #completed
* medicationReference = Reference(urn:uuid:2f861880-31d2-4969-a2ce-b78f750f430e)
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectivePeriod.start = "2021-12-04"
* note.text = "texte libre"

Instance: MedicationAdministration-Exemple-4
InstanceOf: MedicationAdministration
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-administration-document"
* identifier.system = "urn:oid:1.2.3.999"
* identifier.value = "AADC9C14-F1CA-4177-B2C8-A5178D5B3CA0"
* status = #completed
* medicationReference = Reference(urn:uuid:26349c4d2-bdba-41b1-b5d6-1ce8c359205d)
* subject = Reference(urn:uuid:00f54e2e-22f2-4162-87b4-4826d855feac)
* effectivePeriod.start = "2021-12-04"
* note.text = "texte libre"

Instance: Medication-Exemple-3
InstanceOf: Medication
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-document"
* extension.url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-productname"
* extension.valueString = "ROSUVASTATINE EG 5 mg, comprimé pelliculé"
* code = $terminologie-atc#69473265 "ROSUVASTATINE EG 5 mg, comprimé pelliculé"

Instance: Medication-Exemple-4
InstanceOf: Medication
Usage: #inline
* meta.profile = "https://interop.esante.gouv.fr/ig/document/core/StructureDefinition/fr-medication-document"
* extension.url = "https://profiles.ihe.net/PHARM/MPD/StructureDefinition/ihe-ext-medication-productname"
* extension.valueString.extension.url = "http://hl7.org/fhir/StructureDefinition/data-absent-reason"
* extension.valueString.extension.valueCode = #unknown
* code = $terminologie-cisis#GEN-092.03.01 "Autre(s) traitement(s)"